class MetricasIndicador:
  ...
  def _tempo_medio_em_horas_servicos_concluidos(self):
    if self._tempo_medio_em_segundos_servicos_concluidos() > 0:
      return self._tempo_medio_em_segundos_servicos_concluidos() / 3600
    
    return 0
  
  def _tempo_medio_em_minutos_servicos_concluidos(self):
    if self._tempo_medio_em_segundos_servicos_concluidos() > 0:
      return int(self._tempo_medio_em_segundos_servicos_concluidos() % 3600 // 60)
  ...
  def _tempo_medio_servicos_concluidos(self):
    if self.indicador.tipo_de_tempo_limite == 'horas':
      return f"{int(self._tempo_medio_em_horas_servicos_concluidos()):02}:{self._tempo_medio_em_minutos_servicos_concluidos():02}"
  ...
  def _percentual_tempo_servicos_concluidos(self):
    if self.indicador.tipo_de_tempo_limite == 'horas':
      if self.indicador.tempo_limite and self._tempo_medio_em_horas_servicos_concluidos() > 0:
        return round(self._tempo_medio_em_horas_servicos_concluidos() / self.indicador.tempo_limite, 2) * 100
